...
A Retrieval-Augmented Generation (RAG) chatbot designed to answer questions about Motor Laws. It extracts text from local PDF documents, stores them in a Pinecone serverless vector index, and uses a cloud-based LLM (Groq) to provide factually accurate, hallucination-free legal answers.

## Features
- **Offline Embeddings**: Uses HuggingFace (`all-MiniLM-L6-v2`) to generate document embeddings completely locally.
- **Cloud Vector Database**: Uses Pinecone (`legal-rag`) for fast, serverless vector storage and retrieval.
...
- **Stateful API**: A production-ready FastAPI backend with rate limiting, per-key authentication, CORS, and an SQLite-backed session store to safely maintain chat memory across restarts and workers.
- **Evaluation Framework**: An LLM-as-a-Judge script scores the bot on answer accuracy and reports retrieval metrics (hit-rate@k, MRR) over a regenerable benchmark using majority voting.
...
- `static/`: Contains the premium Vanilla JS web interface with built-in API key support and XSS protection.
- `learn.md`: A learning log of architectural decisions and concepts.
- `.github/workflows/eval.yml`: Automated CI workflow for running tests and benchmarks.
...
3. **Configure API Keys**
   ...
   - Add your Groq API key, Pinecone API key, and optionally a HuggingFace token:
     ```env
...
docker run -p 8000:8000 -v ${PWD}/sessions.db:/app/sessions.db --env-file .env legal-rag
```

Vectors live in the Pinecone cloud index, so containers don't hold index data — they just need `PINECONE_API_KEY` (and `GROQ_API_KEY`) at runtime. Be sure to map a volume for `sessions.db` to persist chat histories. Run `python ingest.py --reset` once locally (or from any container) to populate the index before first use.