...
class SessionStore:
    def __init__(self, path: str):
        ...
        self._conn.execute(
            ...
            " id TEXT PRIMARY KEY, history TEXT, created_at REAL)"
        )
        self._conn.commit()
    ...
    def create(self, session_id: str) -> None:
        with self._lock:
            ...
            self._conn.execute(
                "INSERT OR IGNORE INTO sessions (id, history, created_at) VALUES (?, ?, ?)",
                (session_id, json.dumps([]), time.time()),
            )
            self._conn.commit()
    ...
    def save(self, session_id: str, history: list) -> None:
        ...
        with self._lock:
            self._conn.execute(
                "UPDATE sessions SET history=? WHERE id=?", (payload, session_id)
            )
            self._conn.commit()
